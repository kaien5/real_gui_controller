# -*- coding: utf-8 -*-

# Form implementation generated from reading ui file 'main_gui.ui'
#
# Created by: PyQt5 UI code generator 5.15.7
#
# WARNING: Any manual changes made to this file will be lost when pyuic5 is
# run again.  Do not edit this file unless you know what you are doing.


from PyQt5 import QtCore, QtGui, QtWidgets


class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        MainWindow.setObjectName("MainWindow")
        MainWindow.resize(1896, 1023)
        self.centralwidget = QtWidgets.QWidget(MainWindow)
        self.centralwidget.setObjectName("centralwidget")
        self.settings_mainWindow_frame = QtWidgets.QFrame(self.centralwidget)
        self.settings_mainWindow_frame.setGeometry(QtCore.QRect(30, 10, 241, 981))
        self.settings_mainWindow_frame.setFrameShape(QtWidgets.QFrame.Box)
        self.settings_mainWindow_frame.setFrameShadow(QtWidgets.QFrame.Plain)
        self.settings_mainWindow_frame.setObjectName("settings_mainWindow_frame")
        self.start_button = QtWidgets.QPushButton(self.settings_mainWindow_frame)
        self.start_button.setEnabled(False)
        self.start_button.setGeometry(QtCore.QRect(20, 900, 91, 51))
        palette = QtGui.QPalette()
        brush = QtGui.QBrush(QtGui.QColor(0, 193, 0))
        brush.setStyle(QtCore.Qt.SolidPattern)
        palette.setBrush(QtGui.QPalette.Active, QtGui.QPalette.ButtonText, brush)
        brush = QtGui.QBrush(QtGui.QColor(0, 193, 0))
        brush.setStyle(QtCore.Qt.SolidPattern)
        palette.setBrush(QtGui.QPalette.Inactive, QtGui.QPalette.ButtonText, brush)
        brush = QtGui.QBrush(QtGui.QColor(120, 120, 120))
        brush.setStyle(QtCore.Qt.SolidPattern)
        palette.setBrush(QtGui.QPalette.Disabled, QtGui.QPalette.ButtonText, brush)
        self.start_button.setPalette(palette)
        font = QtGui.QFont()
        font.setPointSize(14)
        font.setBold(True)
        font.setWeight(75)
        self.start_button.setFont(font)
        self.start_button.setObjectName("start_button")
        self.frame = QtWidgets.QFrame(self.settings_mainWindow_frame)
        self.frame.setGeometry(QtCore.QRect(20, 420, 201, 271))
        self.frame.setFrameShape(QtWidgets.QFrame.WinPanel)
        self.frame.setFrameShadow(QtWidgets.QFrame.Raised)
        self.frame.setObjectName("frame")
        self.ip_address_microGC = QtWidgets.QLineEdit(self.frame)
        self.ip_address_microGC.setGeometry(QtCore.QRect(100, 40, 91, 21))
        font = QtGui.QFont()
        font.setPointSize(10)
        self.ip_address_microGC.setFont(font)
        self.ip_address_microGC.setObjectName("ip_address_microGC")
        self.microGC_status = QtWidgets.QLabel(self.frame)
        self.microGC_status.setGeometry(QtCore.QRect(10, 100, 181, 21))
        font = QtGui.QFont()
        font.setPointSize(12)
        self.microGC_status.setFont(font)
        self.microGC_status.setAlignment(QtCore.Qt.AlignCenter)
        self.microGC_status.setObjectName("microGC_status")
        self.ip_address_label = QtWidgets.QLabel(self.frame)
        self.ip_address_label.setGeometry(QtCore.QRect(10, 40, 81, 21))
        font = QtGui.QFont()
        font.setPointSize(12)
        self.ip_address_label.setFont(font)
        self.ip_address_label.setObjectName("ip_address_label")
        self.microGC = QtWidgets.QLabel(self.frame)
        self.microGC.setGeometry(QtCore.QRect(10, 10, 181, 21))
        font = QtGui.QFont()
        font.setPointSize(13)
        font.setBold(True)
        font.setWeight(75)
        self.microGC.setFont(font)
        self.microGC.setAlignment(QtCore.Qt.AlignCenter)
        self.microGC.setObjectName("microGC")
        self.sequence_label = QtWidgets.QLabel(self.frame)
        self.sequence_label.setGeometry(QtCore.QRect(10, 70, 131, 21))
        font = QtGui.QFont()
        font.setPointSize(11)
        self.sequence_label.setFont(font)
        self.sequence_label.setObjectName("sequence_label")
        self.sequence_number = QtWidgets.QSpinBox(self.frame)
        self.sequence_number.setEnabled(False)
        self.sequence_number.setGeometry(QtCore.QRect(149, 70, 42, 22))
        self.sequence_number.setMinimum(1)
        self.sequence_number.setObjectName("sequence_number")
        self.connect_button_micro_GC = QtWidgets.QPushButton(self.frame)
        self.connect_button_micro_GC.setGeometry(QtCore.QRect(10, 140, 81, 51))
        font = QtGui.QFont()
        font.setPointSize(10)
        font.setBold(True)
        font.setWeight(75)
        self.connect_button_micro_GC.setFont(font)
        self.connect_button_micro_GC.setObjectName("connect_button_micro_GC")
        self.load_button = QtWidgets.QPushButton(self.frame)
        self.load_button.setEnabled(False)
        self.load_button.setGeometry(QtCore.QRect(110, 140, 81, 51))
        font = QtGui.QFont()
        font.setPointSize(10)
        font.setBold(True)
        font.setWeight(75)
        self.load_button.setFont(font)
        self.load_button.setObjectName("load_button")
        self.microGC_settings_button = QtWidgets.QPushButton(self.frame)
        self.microGC_settings_button.setEnabled(False)
        self.microGC_settings_button.setGeometry(QtCore.QRect(10, 200, 181, 61))
        font = QtGui.QFont()
        font.setPointSize(10)
        font.setBold(True)
        font.setWeight(75)
        self.microGC_settings_button.setFont(font)
        self.microGC_settings_button.setObjectName("microGC_settings_button")
        self.stop_button = QtWidgets.QPushButton(self.settings_mainWindow_frame)
        self.stop_button.setEnabled(False)
        self.stop_button.setGeometry(QtCore.QRect(130, 900, 91, 51))
        palette = QtGui.QPalette()
        brush = QtGui.QBrush(QtGui.QColor(255, 0, 0))
        brush.setStyle(QtCore.Qt.SolidPattern)
        palette.setBrush(QtGui.QPalette.Active, QtGui.QPalette.ButtonText, brush)
        brush = QtGui.QBrush(QtGui.QColor(255, 0, 0))
        brush.setStyle(QtCore.Qt.SolidPattern)
        palette.setBrush(QtGui.QPalette.Inactive, QtGui.QPalette.ButtonText, brush)
        brush = QtGui.QBrush(QtGui.QColor(120, 120, 120))
        brush.setStyle(QtCore.Qt.SolidPattern)
        palette.setBrush(QtGui.QPalette.Disabled, QtGui.QPalette.ButtonText, brush)
        self.stop_button.setPalette(palette)
        font = QtGui.QFont()
        font.setPointSize(14)
        font.setBold(True)
        font.setWeight(75)
        self.stop_button.setFont(font)
        self.stop_button.setObjectName("stop_button")
        self.frame_3 = QtWidgets.QFrame(self.settings_mainWindow_frame)
        self.frame_3.setGeometry(QtCore.QRect(20, 20, 201, 321))
        self.frame_3.setFrameShape(QtWidgets.QFrame.WinPanel)
        self.frame_3.setFrameShadow(QtWidgets.QFrame.Raised)
        self.frame_3.setObjectName("frame_3")
        self.frame_2 = QtWidgets.QFrame(self.frame_3)
        self.frame_2.setGeometry(QtCore.QRect(10, 100, 181, 151))
        self.frame_2.setFrameShape(QtWidgets.QFrame.StyledPanel)
        self.frame_2.setFrameShadow(QtWidgets.QFrame.Sunken)
        self.frame_2.setObjectName("frame_2")
        self.indicator_line = QtWidgets.QLineEdit(self.frame_2)
        self.indicator_line.setGeometry(QtCore.QRect(10, 20, 161, 20))
        font = QtGui.QFont()
        font.setPointSize(10)
        self.indicator_line.setFont(font)
        self.indicator_line.setText("")
        self.indicator_line.setMaxLength(32)
        self.indicator_line.setClearButtonEnabled(True)
        self.indicator_line.setObjectName("indicator_line")
        self.label = QtWidgets.QLabel(self.frame_2)
        self.label.setGeometry(QtCore.QRect(10, 0, 55, 16))
        self.label.setObjectName("label")
        self.command_line = QtWidgets.QLineEdit(self.frame_2)
        self.command_line.setGeometry(QtCore.QRect(10, 70, 161, 20))
        font = QtGui.QFont()
        font.setPointSize(10)
        self.command_line.setFont(font)
        self.command_line.setText("")
        self.command_line.setMaxLength(32)
        self.command_line.setClearButtonEnabled(True)
        self.command_line.setObjectName("command_line")
        self.label_2 = QtWidgets.QLabel(self.frame_2)
        self.label_2.setGeometry(QtCore.QRect(10, 50, 61, 16))
        self.label_2.setObjectName("label_2")
        self.label_3 = QtWidgets.QLabel(self.frame_2)
        self.label_3.setGeometry(QtCore.QRect(10, 100, 61, 16))
        self.label_3.setObjectName("label_3")
        self.payload_double = QtWidgets.QDoubleSpinBox(self.frame_2)
        self.payload_double.setGeometry(QtCore.QRect(10, 120, 161, 22))
        font = QtGui.QFont()
        font.setPointSize(10)
        self.payload_double.setFont(font)
        self.payload_double.setButtonSymbols(QtWidgets.QAbstractSpinBox.NoButtons)
        self.payload_double.setMinimum(-1000.0)
        self.payload_double.setMaximum(1000.0)
        self.payload_double.setObjectName("payload_double")
        self.connect_button_labview = QtWidgets.QPushButton(self.frame_3)
        self.connect_button_labview.setGeometry(QtCore.QRect(10, 40, 81, 51))
        font = QtGui.QFont()
        font.setPointSize(10)
        font.setBold(True)
        font.setWeight(75)
        self.connect_button_labview.setFont(font)
        self.connect_button_labview.setObjectName("connect_button_labview")
        self.disconnect_button_labview = QtWidgets.QPushButton(self.frame_3)
        self.disconnect_button_labview.setEnabled(False)
        self.disconnect_button_labview.setGeometry(QtCore.QRect(110, 40, 81, 51))
        font = QtGui.QFont()
        font.setPointSize(10)
        font.setBold(True)
        font.setWeight(75)
        self.disconnect_button_labview.setFont(font)
        self.disconnect_button_labview.setObjectName("disconnect_button_labview")
        self.send_button_labview = QtWidgets.QPushButton(self.frame_3)
        self.send_button_labview.setEnabled(False)
        self.send_button_labview.setGeometry(QtCore.QRect(10, 260, 181, 51))
        font = QtGui.QFont()
        font.setPointSize(10)
        font.setBold(True)
        font.setWeight(75)
        self.send_button_labview.setFont(font)
        self.send_button_labview.setObjectName("send_button_labview")
        self.microGC_2 = QtWidgets.QLabel(self.frame_3)
        self.microGC_2.setGeometry(QtCore.QRect(10, 10, 181, 21))
        font = QtGui.QFont()
        font.setPointSize(13)
        font.setBold(True)
        font.setWeight(75)
        self.microGC_2.setFont(font)
        self.microGC_2.setAlignment(QtCore.Qt.AlignCenter)
        self.microGC_2.setObjectName("microGC_2")
        self.tabWidget = QtWidgets.QTabWidget(self.centralwidget)
        self.tabWidget.setGeometry(QtCore.QRect(300, 10, 1491, 461))
        self.tabWidget.setObjectName("tabWidget")
        self.Ch1_FF_tab = QtWidgets.QWidget()
        self.Ch1_FF_tab.setObjectName("Ch1_FF_tab")
        self.Ch1_FF = MplWidget(self.Ch1_FF_tab)
        self.Ch1_FF.setEnabled(False)
        self.Ch1_FF.setGeometry(QtCore.QRect(0, 0, 1591, 441))
        self.Ch1_FF.setObjectName("Ch1_FF")
        self.tabWidget.addTab(self.Ch1_FF_tab, "")
        self.Ch2_FF_tab = QtWidgets.QWidget()
        self.Ch2_FF_tab.setObjectName("Ch2_FF_tab")
        self.Ch2_FF = MplWidget(self.Ch2_FF_tab)
        self.Ch2_FF.setEnabled(False)
        self.Ch2_FF.setGeometry(QtCore.QRect(0, 0, 1591, 441))
        self.Ch2_FF.setObjectName("Ch2_FF")
        self.tabWidget.addTab(self.Ch2_FF_tab, "")
        self.Ch2_BF_tab = QtWidgets.QWidget()
        self.Ch2_BF_tab.setObjectName("Ch2_BF_tab")
        self.Ch2_BF = MplWidget(self.Ch2_BF_tab)
        self.Ch2_BF.setEnabled(False)
        self.Ch2_BF.setGeometry(QtCore.QRect(0, 0, 1591, 441))
        self.Ch2_BF.setObjectName("Ch2_BF")
        self.tabWidget.addTab(self.Ch2_BF_tab, "")
        self.last_method = QtWidgets.QLabel(self.centralwidget)
        self.last_method.setGeometry(QtCore.QRect(960, 0, 271, 31))
        font = QtGui.QFont()
        font.setPointSize(16)
        font.setBold(True)
        font.setWeight(75)
        self.last_method.setFont(font)
        self.last_method.setAlignment(QtCore.Qt.AlignCenter)
        self.last_method.setObjectName("last_method")
        self.tabWidget_2 = QtWidgets.QTabWidget(self.centralwidget)
        self.tabWidget_2.setGeometry(QtCore.QRect(300, 470, 1491, 521))
        self.tabWidget_2.setObjectName("tabWidget_2")
        self.tab_4 = QtWidgets.QWidget()
        self.tab_4.setObjectName("tab_4")
        self.electron_image = MplWidget(self.tab_4)
        self.electron_image.setEnabled(False)
        self.electron_image.setGeometry(QtCore.QRect(990, 0, 501, 501))
        self.electron_image.setObjectName("electron_image")
        self.mass_spectrum = MplWidget(self.tab_4)
        self.mass_spectrum.setEnabled(False)
        self.mass_spectrum.setGeometry(QtCore.QRect(-10, 0, 1101, 501))
        self.mass_spectrum.setObjectName("mass_spectrum")
        self.tabWidget_2.addTab(self.tab_4, "")
        self.Table_tab = QtWidgets.QWidget()
        self.Table_tab.setObjectName("Table_tab")
        self.chromatogram_table = QtWidgets.QTableWidget(self.Table_tab)
        self.chromatogram_table.setEnabled(False)
        self.chromatogram_table.setGeometry(QtCore.QRect(10, 10, 1461, 531))
        self.chromatogram_table.setAutoFillBackground(True)
        self.chromatogram_table.setEditTriggers(QtWidgets.QAbstractItemView.NoEditTriggers)
        self.chromatogram_table.setObjectName("chromatogram_table")
        self.chromatogram_table.setColumnCount(0)
        self.chromatogram_table.setRowCount(0)
        self.tabWidget_2.addTab(self.Table_tab, "")
        self.reset_chromatogram = QtWidgets.QPushButton(self.centralwidget)
        self.reset_chromatogram.setGeometry(QtCore.QRect(1800, 80, 93, 28))
        font = QtGui.QFont()
        font.setPointSize(10)
        font.setBold(True)
        font.setWeight(75)
        self.reset_chromatogram.setFont(font)
        self.reset_chromatogram.setObjectName("reset_chromatogram")
        self.zoom_box = QtWidgets.QCheckBox(self.centralwidget)
        self.zoom_box.setGeometry(QtCore.QRect(1800, 130, 91, 21))
        font = QtGui.QFont()
        font.setPointSize(9)
        self.zoom_box.setFont(font)
        self.zoom_box.setChecked(True)
        self.zoom_box.setObjectName("zoom_box")
        self.line_box = QtWidgets.QCheckBox(self.centralwidget)
        self.line_box.setGeometry(QtCore.QRect(1800, 150, 91, 21))
        font = QtGui.QFont()
        font.setPointSize(9)
        self.line_box.setFont(font)
        self.line_box.setObjectName("line_box")
        self.enable_plots = QtWidgets.QCheckBox(self.centralwidget)
        self.enable_plots.setEnabled(False)
        self.enable_plots.setGeometry(QtCore.QRect(1800, 110, 91, 21))
        font = QtGui.QFont()
        font.setPointSize(9)
        self.enable_plots.setFont(font)
        self.enable_plots.setObjectName("enable_plots")
        MainWindow.setCentralWidget(self.centralwidget)
        self.menuBar = QtWidgets.QMenuBar(MainWindow)
        self.menuBar.setGeometry(QtCore.QRect(0, 0, 1896, 21))
        self.menuBar.setObjectName("menuBar")
        self.menu_Open = QtWidgets.QMenu(self.menuBar)
        self.menu_Open.setObjectName("menu_Open")
        MainWindow.setMenuBar(self.menuBar)
        self.action_Open = QtWidgets.QAction(MainWindow)
        self.action_Open.setObjectName("action_Open")
        self.menu_Open.addAction(self.action_Open)
        self.menuBar.addAction(self.menu_Open.menuAction())

        self.retranslateUi(MainWindow)
        self.tabWidget.setCurrentIndex(0)
        self.tabWidget_2.setCurrentIndex(0)
        QtCore.QMetaObject.connectSlotsByName(MainWindow)

    def retranslateUi(self, MainWindow):
        _translate = QtCore.QCoreApplication.translate
        MainWindow.setWindowTitle(_translate("MainWindow", "GC Chiral MS"))
        self.start_button.setText(_translate("MainWindow", "START"))
        self.ip_address_microGC.setText(_translate("MainWindow", "127.0.0.1"))
        self.microGC_status.setText(_translate("MainWindow", "Status unknown"))
        self.ip_address_label.setText(_translate("MainWindow", "IP Address:"))
        self.microGC.setText(_translate("MainWindow", "Micro GC"))
        self.connect_button_micro_GC.setText(_translate("MainWindow", "Connect"))
        self.load_button.setText(_translate("MainWindow", "LOAD"))
        self.microGC_settings_button.setText(_translate("MainWindow", "MicroGC information"))
        self.stop_button.setText(_translate("MainWindow", "STOP"))
        self.label.setText(_translate("MainWindow", "Indicator"))
        self.label_2.setText(_translate("MainWindow", "Command"))
        self.label_3.setText(_translate("MainWindow", "Payload"))
        self.connect_button_labview.setText(_translate("MainWindow", "Connect"))
        self.disconnect_button_labview.setText(_translate("MainWindow", "Disconnect"))
        self.send_button_labview.setText(_translate("MainWindow", "Send Message"))
        self.microGC_2.setText(_translate("MainWindow", "Mass Spectrometer"))
        self.tabWidget.setTabText(self.tabWidget.indexOf(self.Ch1_FF_tab), _translate("MainWindow", "Ch1 (FF)"))
        self.tabWidget.setTabText(self.tabWidget.indexOf(self.Ch2_FF_tab), _translate("MainWindow", "Ch2 (FF)"))
        self.tabWidget.setTabText(self.tabWidget.indexOf(self.Ch2_BF_tab), _translate("MainWindow", "Ch2 (BF)"))
        self.last_method.setText(_translate("MainWindow", "Method name"))
        self.tabWidget_2.setTabText(self.tabWidget_2.indexOf(self.tab_4), _translate("MainWindow", "TOF spectrum and electron image"))
        self.tabWidget_2.setTabText(self.tabWidget_2.indexOf(self.Table_tab), _translate("MainWindow", "Table"))
        self.reset_chromatogram.setText(_translate("MainWindow", "Reset"))
        self.zoom_box.setText(_translate("MainWindow", "Zoom"))
        self.line_box.setText(_translate("MainWindow", "Line"))
        self.enable_plots.setText(_translate("MainWindow", "Enable plots"))
        self.menu_Open.setTitle(_translate("MainWindow", "File"))
        self.action_Open.setText(_translate("MainWindow", "&Open"))
from mplwidget import MplWidget


if __name__ == "__main__":
    import sys
    app = QtWidgets.QApplication(sys.argv)
    MainWindow = QtWidgets.QMainWindow()
    ui = Ui_MainWindow()
    ui.setupUi(MainWindow)
    MainWindow.show()
    sys.exit(app.exec_())
